...
function renderStudyCard() {
  ...
  const eligibleCards = getStudyEligibleCards();
  if (studyOrder.length !== eligibleCards.length) {
    const currentDisplayedCard = getCurrentStudyCard();
    const keepShowingKnownCard = deferKnownOmissionForCurrent && currentDisplayedCard?.known === true;
    if (!keepShowingKnownCard) {
      rebuildStudyOrder();
    }
  }
